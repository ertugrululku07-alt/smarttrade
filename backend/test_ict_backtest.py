"""Test ICT/SMC Full Backtest v5.0 — Real ICT Analysis"""
import requests
import json
import time
...
print("=" * 80)
print("ICT/SMC v5.0 — REAL ICT ANALYSIS (BOS/CHoCH + OB + FVG + Sweep) — 7 COINS x 30 DAYS")
print("=" * 80)

results = []
for coin in coins:
    try:
        r = requests.post(f"{API}/backtest/ict-full-backtest", json={
            "symbol": coin, "days": 30,
        ...
        # Show last 8 trades with SL/TP levels
        ...
        if trade_list:
            ...
            for t in trade_list[-8:]:
                ...
                print(f"    {t['direction']:5s} | {t['entry_time'][:16]} | E:{t['entry_price']:.4f} SL:{sl_p:.4f} TP:{tp_p:.4f} | PnL: {icon2}${t['pnl']:.2f} ({icon2}{t['pnl_pct']:.1f}%) | {t['exit_reason']}")
        ...
        print(f"\n  {coin}: ERROR — {e}")
        results.append({'coin': coin, 'pnl': 0, 'trades': 0, 'wr': 0, 'pf': 0, 'error': str(e)})
