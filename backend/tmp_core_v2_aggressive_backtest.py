...
# Aggressive backtest overrides (target-seeking preset)
ICTFullBacktest.MIN_RR = 1.35
ICTFullBacktest.MAX_LOSS_DOLLAR = 6.5
ICTFullBacktest.NOTIONAL_CAP = 350.0
ICTFullBacktest.BALANCE_PCT = 0.20
ICTFullBacktest.POI_PROXIMITY_ATR = 2.0
ICTFullBacktest.COOLDOWN_BARS = 2
ICTFullBacktest.TRAIL_START_PCT = 0.018
ICTFullBacktest.TRAIL_DIST_PCT = 0.007

symbols = get_all_usdt_pairs()[:CORE_V2_SYMBOL_LIMIT]
results = []
t0 = time.time()
print(f'START aggressive symbols={len(symbols)} days=180')
...
# Portfolio replay (shared balance, max 6 concurrent)
trades = []
for r in oks:
    sym = r.get('symbol')
...
out = {
    'meta': {
        'preset': 'core_v2_aggressive_backtest_preset_v1',
        'days': 180,
        'initial_balance': 1000.0,
        'leverage': 10,
...
path = 'logs/core_v2_aggressive_backtest_180d.json'
