...
if not DATABASE_URL:
    print("[WARN] DATABASE_URL not found! Using local fallback.")
...
print(f"[NET] Attempting to initialize DB engine (URL prefix: {DATABASE_URL.split(':')[0]})")

try:
    engine = create_engine(DATABASE_URL)
    SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
except Exception as e:
    print(f"[ALERT] CRITICAL: Engine creation failed: {e}")
